...
class Admin extends CI_Controller
{
    ...
    public function data_konflik()
    {
        ...
        $data['akun'] = $this->db->get_where('tb_user', ['id_user' => $this->session->userdata('id_user')])->row_array();
        $data['jenis'] = $this->M_admin->getKonflik();
        // $data['user'] = $this->db->get('tb_user')->result_array();
        $this->load->view('templates/template_header', $data);
        $this->load->view('templates/template', $data);
        $this->load->view('templates/template_footer', $data);
    ...
    {
        $result['error'] = TRUE;
        $this->form_validation->set_rules('nama_jenis', 'nama_jenis', 'required|trim|is_unique[tb_jenis_konflik.nama_jenis]', [
            'is_unique' => 'This Jenis Konflik is already registered!'
        ]);
        if ($this->form_validation->run() == FALSE) {
            $this->data_konflik();
        } else {
            //mengirim post ke model
            $this->M_admin->save_konflik($_POST);
    ...
    {
        echo json_encode($this->M_admin->get_konflik_byId($_POST['id_jenis']));
    }

    public function saveUpdate_jenis()
    {
        
        $result['error'] = TRUE;
        $this->form_validation->set_rules('nama_jenis', 'nama_jenis', 'required|trim');
        if ($this->form_validation->run() == FALSE) {
            $data['title'] = 'Data Jenis Konflik';
            $data['content'] = 'data_table/data_konflik';
            $data['akun'] = $this->db->get_where('tb_user', ['id_user' => $this->session->userdata('id_user')])->row_array();
            $data['jenis'] = $this->M_admin->getKonflik();
            $data['jenis'] = $this->db->get('tb_jenis_konflik')->result_array();
    ...
    public function delete_konflik($id_jenis)
    {
        $where = array('id_jenis' => $id_jenis);
        $this->M_admin->delete_data($where, 'tb_jenis_konflik');
        $this->session->set_flashdata('konflik', 'Deleted');
        redirect('Admin/data_konflik');
    }
